...
if CLIENT then
	...
	net.Receive(NET_SPAWN_LEAVE, function()
		...
		if is_join then
			...
			if last_seen_diff == -1 then
				chat.AddText(black_color, " ▸ ", white_color, "Подключился ", cyan_color, "впервые", white_color, "!")
			else
				chat.AddText(black_color, " ▸ ", white_color, "Последний онлайн ", cyan_color, seen_date, white_color, " - ", teal_color, os.date("%H:%M", last_seen_time), gray_color, formatted_diff)
			end
			...
			if reason == "Gave up connecting" then
				...
				chat.AddText(red_color, " ● ", ply_col, name, gray_color, formatted_id, white_color, "", red_color, "вышел", white_color, " с сервера", red_color, " (" .. reason .. ")")
			end
		end
	end)
